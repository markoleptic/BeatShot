...
String ID3v2::Tag::genre() const
{
  ...
  for(StringList::Iterator it = fields.begin(); it != fields.end(); ++it) {
    ...
    if(std::find(genresList.begin(), genresList.end(), *it) == genresList.end())
        genresList.append(*it);
  }
...
ID3v2::Footer *ID3v2::Tag::footer() const
{
  return d->footer;
}
...
void ID3v2::Tag::downgradeFrames(FrameList *frames, FrameList *newFrames) const
{
  ...
  if(frameTCON) {
    StringList genresList = frameTCON->fieldList();
    String combined;
    String genreText;
    ...
    for(StringList::ConstIterator it = genresList.begin(); it != genresList.end(); ++it) {
      bool ok = false;
      int number = it->toInt(&ok);
