...
void FSteamFixModule::AddButtonToBlueprintEditor()
{
	...
	ToolbarExtender->AddToolBarExtension("Debugging", EExtensionHook::After, PluginCommands, FToolBarExtensionDelegate::CreateRaw(this, &FSteamFixModule::AddToolbarExtension));
...
void FSteamFixModule::StartupModule()
{
	...
	PluginCommands->MapAction(FSteamFixCommands::Get().ButtonAction, FExecuteAction::CreateRaw(this, &FSteamFixModule::SteamButtonClicked), FCanExecuteAction());
...
void FSteamFixModule::SteamButtonClicked()
{
	...
	auto ProcHandle = FPlatformProcess::CreateProc(*CmdPath, *CmdParams, bLaunchDetached, bLaunchHidden, bLaunchReallyHidden, OutProcessID, PriorityModifier, OptionalWorkingDirectory, PipeWriteChild);

	UE_LOG(LogSteamFix, Log, TEXT("Cmd path: %s\n\tProject path: %s\n\tProcessID: %d\n\tCmd Params: %s"), *CmdPath, *ProjectFile, *OutProcessID, *CmdParams);
}
...
void FSteamFixModule::RegisterMenus()
{
...

	{
		UToolMenu* ToolbarMenu = UToolMenus::Get()->ExtendMenu("LevelEditor.LevelEditorToolBar.PlayToolBar");
		{
			...
			{
				FToolMenuEntry& Entry = Section.AddEntry(FToolMenuEntry::InitToolBarButton(FSteamFixCommands::Get().ButtonAction));
