...
#include "Tests/BeatShotGameModeFunctionalTest.h"
...
{
	StartStep(UEnum::GetDisplayValueAsText(GameModeConfigs[CurrentIndex].DefiningConfig.BaseGameMode).ToString());
	*GameModeConfig = GameModeConfigs[CurrentIndex];
	...
	GetWorldTimerManager().SetTimer(BeatTimer, this, &ThisClass::OnAudioAnalyzerBeat, BeatFrequency, true,
		InitialDelay);
	GetWorldTimerManager().SetTimer(DestroyTimer, this, &ThisClass::DestroyTargets, BeatFrequency, true,
		InitialDelay + BeatFrequency / 2.f);
}
