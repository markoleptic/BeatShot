...
void AGameModeActorBase::InitializeGameModeActor()
{
	...
	Cast<ADefaultCharacter>(Cast<ADefaultPlayerController>(UGameplayStatics::GetPlayerController(GetWorld(), 0))->GetPawn())->Gun->
...
void AGameModeActorBase::LoadMatchingPlayerScores()
{
...
TArray<FPlayerScore> AGameModeActorBase::GetPlayerScores()
{
	/** don't save scores if score is zero */
	if (CurrentPlayerScore.Score <= 0 ||
		(CurrentPlayerScore.GameModeActorName == EGameModeActorName::Custom &&
...
void AGameModeActorBase::OnPlayerSettingsChanged(const FPlayerSettings& PlayerSettings)
{
	bShowStreakCombatText = PlayerSettings.bShowStreakCombatText;
	CombatTextFrequency = PlayerSettings.CombatTextFrequency;
}
