...
FVector ATargetSpawner::GetNextTargetSpawnLocation(const EBoundsScalingMethod BoundsScalingMethod, const float NewTargetScale)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("OpenLocations is empty."));
		bSkipNextSpawn = true;
		return GetBoxOrigin();
	}
	bSkipNextSpawn = false;
	...
	if (BSConfig.AIConfig.bEnableReinforcementLearning)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Unable to Spawn Location suggested by RLAgent."));
	}
...
FVectorCounter ATargetSpawner::GetVectorCounterFromPoint(const FVector& Point) const
{
	...
	if (Index == INDEX_NONE)
	{
		UE_LOG(LogTemp, Warning, TEXT("Point Not Found in SpawnCounter"));
...
void ATargetSpawner::UpdateRLAgentReward(const FVector& WorldLocation, const bool bHit)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Location not found in ActiveTargetPairs %s"), *WorldLocation.ToString());
		return;
	}
...
void ATargetSpawner::UpdateRLAgent()
{
	...
	while (!TargetPairs.IsEmpty())
	{
		...
		const int32 Action2Index = ReinforcementLearningComponent->GetMaxActionIndex(State2Index);

		/* Don't update RLAgent if something went wrong */
		if (Action2Index < SpawnCounter.Num() && Action2Index != INDEX_NONE)
		{
			ReinforcementLearningComponent->UpdateEpisodeRewards(TargetPair.Reward);
			ReinforcementLearningComponent->UpdateQTable(FAlgoInput(StateIndex, State2Index, ActionIndex, Action2Index, TargetPair.Reward));
		}
		else
		{
...
void ATargetSpawner::ToggleDebug_SpawnBox(const bool bShow)
{
	bShowDebug_SpawnBox = bShow;
}

void ATargetSpawner::ToggleDebug_SpawnMemory(const bool bShow)
{
	bShowDebug_SpawnMemory = bShow;
}

void ATargetSpawner::ToggleRLAgentWidget(const bool bShow)
{
	if (!ReinforcementLearningComponent->IsActive())
	{
		return;
	...
	bShowDebug_RLAgentWidget = bShow;
